...
func (ec *executionContext) resolveEntity(
	...
	case "Profile":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Profile": %w`, err)
		}
		switch resolverName {
		...
		case "findProfileByUserID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findProfileByUserID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindProfileByUserID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Profile": %w`, err)
			}
...
func entityResolverNameForProfile(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"userID\" for Profile", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Profile", ErrTypeNotFound))
			break
		}
		return "findProfileByUserID", nil
	}
	return "", fmt.Errorf("%w for Profile due to %v", ErrTypeNotFound,
